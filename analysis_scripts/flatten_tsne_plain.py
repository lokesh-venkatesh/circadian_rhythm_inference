...
# Compute day_of_year as a fraction (0-1) for color-coding (not used anymore)
...
sc = ax.scatter(latent_3d_tsne[:, 0], latent_3d_tsne[:, 1], latent_3d_tsne[:, 2],
                color='royalblue', s=12, alpha=0.8)
# No colorbar
...
plt.tight_layout()
plt.savefig("images/latent_space_tsne_3d_plain.png", dpi=300)
plt.close()
...
fig_tsne_plotly.update_traces(marker=dict(size=4, opacity=0.7))
fig_tsne_plotly.write_html("images/latent_space_tsne_3d_interactive_plain.html")
...
for (i, j), (xlabel, ylabel) in zip(pairs, pair_labels):
    ...
    plt.tight_layout()
    plt.savefig(f"images/latent_space_tsne_{i+1}_{j+1}_plain.png", dpi=300)
    plt.close()