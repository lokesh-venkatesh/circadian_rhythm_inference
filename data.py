...
if prior_dist_type == 'Seasonal':
    ...
    # --- Plotting block for all four stages as subplots (last 10 years only) ---
    ...
    # For 'Normal' case, skip deseasonalization
    # --- Plotting block for three stages as subplots (last 10 years only) ---
    last_10_years = df_mst.index >= (df_mst.index.max() - pd.DateOffset(years=10))
    fig, axes = plt.subplots(3, 1, figsize=(28, 9), sharex=True)
    ...
    climate_adjusted = df_mst['Climate Adjusted']
    offset = round(climate_adjusted.mean(), 8)
    scale = round(climate_adjusted.std(), 8)
    normalised = (climate_adjusted - offset) / scale

    axes[2].plot(df_mst.index[last_10_years], normalised[last_10_years], color='tab:red')
    axes[2].set_title('After Normalisation - Last 10 Years')
